...
| `frontend/src/views/` | Pages: `Home` (merged System Environment page: hardware section (CPU / memory / GPU / CUDA) plus the embedded 「运行环境」 Runtime Environment section — llama.cpp per-component status (main program + CUDA runtime) with resumable download / custom directory, owned by `RuntimeSection.vue`; also hosts the quick-start onboarding checklist; `/runtime` redirects to `/` preserving `?section=runtime`), `Chat` (local chat: direct SSE streaming to llama-server; the page always renders — sending a message auto-starts the service when stopped (guided pre-checks for missing models / runtime), then unloads other loaded models before streaming so the selected model is the only one in memory), `Models` (merged model page: shell with two tabs — `Download` tab (default, route `/models/download`: HF Mirror / ModelScope search; backed by `downloadsState.ts`) + `My Models` tab (route `/models/local`: model list; merges the model download path and imported external directories, each model labeled with its source dir); `/downloads` redirects), `ModelDetail` (model detail at `/models/model/:modelId`: file list + description; not in sidebar), `ModelSettings` (standalone route page for per-model inference params; not in sidebar), `Api` (service start/stop + inference speed monitor (token TPS / uptime) + service log console; former `Monitor` page merged in and redirected), `Settings` (theme / language / download source / directories: llama.cpp & model download paths + external import dirs / tray / API-route mode / check for updates), `Docs` (in-app bilingual tutorial page; per-section lazy content with remote-first fetch: GitHub main → disk cache → bundled fallback, source badge + refresh + open-on-GitHub). Sidebar order: Home → Chat → Models → API → Settings → Docs |
...
- **A running `wails dev` session reacts to backend edits**: it hot-reloads Go changes and may regenerate `wailsjs/` bindings or create runtime artifacts (e.g. the remote-docs cache) while other work is in flight. Before staging, check `git status` and disposition such drift deliberately (commit regenerated bindings on their own or revert them); never kill the user's dev session to "clean up".
