...
import { appConfig, loadConfig, setTheme, setDownloadSource } from '../store'
...
describe('store', () => {
  ...
  it('loadConfig 成功时写入主题、llamaCppDir 与 modelsDir', async () => {
    mockGetConfig.mockResolvedValue({ theme: 'light', llamaCppDir: 'C:/llama-cpp', modelsDir: 'D:/models', downloadSource: 'modelscope' })
  ...
  it('loadConfig 后端未返回 modelsDir 时兜底为空串', async () => {
    mockGetConfig.mockResolvedValue({ theme: 'dark', llamaCppDir: 'C:/llama-cpp', modelsDir: '', downloadSource: '' })
  ...
  it('loadConfig 后端未返回 downloadSource 时兜底为 hf', async () => {
    mockGetConfig.mockResolvedValue({ theme: 'dark', llamaCppDir: '', modelsDir: '', downloadSource: '' })
