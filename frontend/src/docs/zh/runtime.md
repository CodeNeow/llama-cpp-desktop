「运行环境」是「本机环境」页中管理推理引擎的标签，与「系统信息」标签并列，负责服务运行所依赖的两样东西：llama.cpp 主程序（llama-server）以及 Windows 上随 CUDA 构建一起安装的 CUDA 运行时（cudart）。

## 状态与组件

该标签以卡片形式展示 llama.cpp 的信息：
...
下载路径本身可以在「偏好设置」页的「llama.cpp 下载路径」行修改，决定新下载的 llama.cpp 安装到哪里。

## CUDA 兼容性三态显示

「本机环境」页的 CUDA 卡片（「系统信息」标签，仅 Windows 上检测到 NVIDIA 显卡时显示）中有一行「CUDA 兼容性」，用三种状态说明你的显卡与 CUDA 运行时是否匹配：
